function [out] = huber_sparse_l0(A,lambda, b,num, c, hist)
tstart=tic;
L = max(max(abs(A'*A)));
...
A1 = A;
[o,x,~,new_objective,~] = ABPG_gain(L,A,x,b,1e-6, c,hist);
out.F = o.F;


k = 1;
idx = 1;
objective = objective_value(A,x,b,lambda,c);
while 1
    % BGP step
    [o,y_new,~] = iteration(L,A1,x,b,lambda,num,c);
    ...
    new_objective = objective_value(A1,x_new,b,lambda,c);

    if abs(objective- new_objective)<1e-6
        % recover x_new
...
function [out,x,z1,new_objective,ind] = ABPG_gain(L,A,x,b,tol,c,hist)
rho = 1.2;
% out.x = [];
...
for k = 1:2000
    ...
    x = x1;
    G = G1;
    [new_objective,a] = func_grad(A1,x1,b,c);
...
function [out,x,new_objective] = iteration(L,A,x,b,lambda,num,c)
[objective,a] = func_grad(A,x,b,c);
for i = 1:num
    out.F{i} = objective + lambda*sum(x~=0);
    % under the fixed lambda, updated the x
    x = prox_map(x,a,L);
    [new_objective,a] = func_grad(A,x,b,c);
    objective = new_objective;
end
end
...
function [obj,grad] = func_grad(A,x,b,c)
residual = A*x-b;
grad = zeros(length(x),1);
obj = 0;
for i = 1:length(b)
    if abs(residual(i)) > c
        obj = obj + c * abs(residual(i)) - c^2/2;
        % grad = grad + A(i,:)' * (threshold * residual(i)/(abs(residual(i))));
        residual(i) = c * residual(i)/(abs(residual(i)));
    else
        obj = obj + residual(i)^2/2;
        % grad = grad + A(i,:)' * residual(i);
    end
...
function [obj] = objective_value(A,x,b,lambda,c)
obj = lambda*sum(x~=0);
residual = A*x-b;
for i = 1:length(b)
    if abs(residual(i)) > residual
        obj = obj + c * abs(residual(i)) - c^2/2;
    else
        obj = obj + residual(i)^2/2;
    end
